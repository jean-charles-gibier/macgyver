...
class Perso(Item):
    ...
    def move(self, direction, autorized_pos):
        ...
        if direction == 'droite'\
                and (self.case_x + 1, self.case_y) in autorized_pos:
            self.case_x += 1
        ...
        if direction == 'haut'\
                and (self.case_x, self.case_y - 1) in autorized_pos:
            self.case_y -= 1
    ...
    def compare_pos(self, other_item):
        if isinstance(other_item, Item) or isinstance(other_item, Perso):
            return (self.value_x, self.value_y)\
                   == (other_item.value_x, other_item.value_y)
        elif isinstance(other_item, tuple):
            return (self.case_x, self.case_y)\
                   == (other_item[0], other_item[1])
        else:
